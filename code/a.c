...
void clip(struct lines mylines)
{
    ...
    // if both points are inside then accept the line and draw
    ...
    // this will contain cases where line maybe totally outside or partially inside
    else {
        ...
        for (i = 0; i < 4; i++) {
            ...
            if (bits[i] == byte[i])
                continue;
            // Otherwise there exists an intersection
